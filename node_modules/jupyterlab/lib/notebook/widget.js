...
var Notebook = (function (_super) {
    ...
    Notebook.prototype.scrollToActiveCell = function () {
        if (this.activeCell) {
            domutils_1.ElementExt.scrollIntoViewIfNeeded(this.node, this.activeCell.node);
        }
    };
    /**
    ...
    Notebook.prototype._ensureFocus = function (force) {
        ...
        if (this.mode === 'edit') {
            ...
            var w = algorithm_1.find(this.layout, function (widget) {
    ...
    Notebook.prototype._evtDrop = function (event) {
        ...
        var factory = model.contentFactory;
        // Insert the copies of the original cells.
        algorithm_1.each(values, function (cell) {
            var value;
            switch (cell.cell_type) {
                case 'code':
                    value = factory.createCodeCell({ cell: cell });
                    break;
                case 'markdown':
                    value = factory.createMarkdownCell({ cell: cell });
                    break;
                default:
                    value = factory.createRawCell({ cell: cell });
                    break;
            }
    ...
    Notebook.prototype._startDrag = function (index, clientX, clientY) {
        ...
        this._drag.start(clientX, clientY).then(function (action) {
            ...
            _this._drag = null;
            algorithm_1.each(toremove, function (widget) { widget.removeClass(DROP_SOURCE_CLASS); });
            if (action === 'none') {
                return;
            }
            var activeCell = cells.at(_this.activeCellIndex);
            algorithm_1.each(toremove, function (widget) {
                _this.model.cells.remove(widget.model);
            });
            _this.activeCellIndex = algorithm_1.ArrayExt.firstIndexOf(algorithm_1.toArray(cells), activeCell);
        });
    };
    /**
